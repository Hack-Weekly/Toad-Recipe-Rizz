<script>
    ...
    let memberSince = new Date($page.data.profile.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
</script>
...
{#if $page.data.username}
    <div class="account-info min-h-[64px] w-full h-32 flex justify-between items-center px-4">
        <div class="left-side flex justify-start items-center gap-x-6 text-xs mt-24">
            <img src="{$page.data.profile.userPicture}" alt="account-pfp" class="w-24 h-24 lg:w-48 lg:h-48 rounded-full">
